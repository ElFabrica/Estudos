...
function App() {
  ...
  const [sumErrorWithCoeficiente, setSumErrorWithCoeficiente] = useState<number | null>();
  const [newW0, setNewW1] = useState<number | null>();
  const [newW1, setNewW2] = useState<number | null>();
  ...
  function handleCalculateErrors(){
    ...
    //sum error
    setSumError(newErrors.reduce((acc,current) => acc + current ,0))
    ...
    <>
      <div>
         ...
         <div className='flex flex-row gap-2'>
          ...
          <button 
          className='bg-gray-400 rounded-sm text-white p-2'
          onClick={handleAddY}> Adicionar X
          </button>
          </div>
